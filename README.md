...
**ブラウザで解く詰将棋トレーナー。SVGの盤面で、王手を続けて玉を詰ます手筋を1手詰から7手詰まで練習できる。相手の受けは最善手を自動で指し、解答状況は手元に残る**
...
王手にならない手や、詰みを逃す手は受け付けず、「その手では詰みません」と指摘して指し直しになる。詰み筋が見えないときはヒントで次の一手のあたりを示し、答えを見ると解答を盤上で再生する。読みを誤ったら「待った」で一手ずつ戻せる。問題は1手詰から7手詰まで13問を用意し、易しい形から段階的に難しくなる。

盤はマウスでもキーボードでも操作でき、矢印キーで升を選んでEnterで動かせる。解いた問題は手元(localStorage)に記録され、進捗バーと一覧の「済」表示でひと目で分かる。問題ごとにURLが変わるので、特定の問題をそのまま共有・ブックマークできる。
...
| 操作             | マウス / クリック                     | キーボード              |
| :--------------- | :------------------------------------ | :---------------------- |
| 駒を選ぶ・動かす | 盤上の駒をクリック → 行き先をクリック | 矢印キーで升を選びEnter |
| 持ち駒を打つ     | 駒台の駒を選ぶ → 空きマスをクリック   | 駒台でTab → Enter       |
| ヒント           | ヒントボタン                          | `H`                     |
| 待った           | 一手戻す                              | `U`                     |
| 初手から         | 今の問題を最初の局面へ戻す            | `R`                     |
| 解答             | 解答の手順を盤上で再生する            | `A`                     |
| 次 / 前の問題    | 次・前の問題へ                        | `N` / `P`               |
| テーマ切替       | テーマボタン(自動・明・暗)            | `T`                     |
| 選択の解除       | ―                                     | `Esc`                   |

解いた手順は「棋譜をコピー」でテキストとして書き出せる。
...
- `src/lib/problems.ts` 1手詰から7手詰までの問題集(全問ソルバーで検証)
- `src/lib/progress.ts` 解答進捗の保存形式とURL共有の符号化(純関数)
- `src/main.ts` SVG盤の描画・操作・トレーナーの進行
- `src/style.css` 盤と駒の配色、タイポグラフィ、レイアウト
- `docs/` アーキテクチャ図
...
- **状態はURLとlocalStorageに置く** — 表示中の問題はURLハッシュに、解いた問題とテーマはlocalStorageに持つ。リロードしても続きから解け、問題のURLをそのまま共有できる。符号化は描画から切り離した純関数にして単体テストで固める。

## 制約

- 収録は1手から7手の詰将棋13問に絞っている。さらに長手数の難問は扱わない。
- 攻め方は王手の連続で詰ます問題のみを対象とし、詰将棋以外の対局や次の一手問題は扱わない。
- 受けの応手は「最も長く粘る手」を選ぶ。詰将棋の慣例(無駄合いをしないなど)の細部までは踏み込まない。
- 詰み探索は全幅のAND-OR探索のため、収録した短手数では即答できるが、長手数の局面では指数的に重くなる。
